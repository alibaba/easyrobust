...
from easyrobust.augmentations import AugMixDataset, AugMixModule
...
def main():
    ...
    dataset_train = AugMixDataset(dataset_train, transforms.Compose([transforms.ToTensor(), 
                                                transforms.Normalize(mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225])]), mixture_width=5, aug_severity=1, img_tuple_num=2, img_size=224)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        images_tuple_1 = input[0]
        images_tuple_1 = [images.cuda() for images in images_tuple_1]
        images_tuple_2 = input[1]
        images_tuple_2 = [images.cuda() for images in images_tuple_2]
        target = target.cuda()

        # with amp_autocast():
        #     logits = model(images_tuple_1[0])
            
        #     imgs_augmix_1 = augmix_module(images_tuple_1)
        #     logits_augmix_1 = model(imgs_augmix_1.detach())

        #     imgs_augmix_2 = augmix_module(images_tuple_2)
        #     logits_augmix_2 = model(imgs_augmix_2.detach())

        #     loss_clean = loss_fn(logits, target)
        #     p_clean, p_aug1, p_aug2 = F.softmax(logits, dim=1), F.softmax(logits_augmix_1, dim=1), F.softmax(logits_augmix_2, dim=1)
        ...
        bx = torch.cat((augmix_module(images_tuple_1), images_tuple_1[0], augmix_module(images_tuple_2)), 0)
        by = target
        
        logits = model(bx.detach())
        l_mix1, l_orig, l_mix2 = torch.split(logits, images_tuple_1[0].size(0))

        loss = loss_fn(l_orig, by)
