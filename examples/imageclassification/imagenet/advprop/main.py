...
parser.add_argument('--decay-milestones', default=[30, 60, 90, 100], type=int, nargs='+', metavar="MILESTONES",
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        # create random target label
        label_offset = torch.randint_like(target, low=0, high=1000)
        attack_target = (target + label_offset) % 1000
        
        model.apply(bn_to_adv_status)
        adv_input = pgd_generator(input, attack_target, model, eps=1/255, attack_steps=1, attack_lr=1/255, random_start_prob=0.4, targeted=True, use_best=False)
        # with amp_autocast():
        #     adv_output = model(adv_input.detach())
        #     loss = loss_fn(adv_output, target)
        
        input = torch.cat([input, adv_input], dim=0)
        target = torch.cat([target, target], dim=0)
        model.apply(bn_to_mix_status)        
        with amp_autocast():
