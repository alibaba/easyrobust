...
class SpeechlmModel(BaseFairseqModel):
    ...
    def forward_features(self, source: torch.Tensor, attack=None) -> torch.Tensor:
        if self.feature_grad_mult > 0:
            ...
            if attack is not None:
                features = self.feature_extractor(source)
                features = GradMultiply.apply(features, 0.5)
            else:
                with torch.no_grad():
                    features = self.feature_extractor(source)
    ...
    def forward(
        ...
        feature_em: Optional[torch.Tensor] = None,
    ...
    def forward_speech(
        ...
        feature_res = None
        if feature_em is None:
            features = self.forward_features(source, attack=feature_em)
        ...
        if features_only:
            return {"x": x, "padding_mask": padding_mask, "features": features, "feature_em": feature_res}
    ...
    def extract_features(
        ...
        **kwargs,
        ...
        return feature, padding_mask, fearure_em
