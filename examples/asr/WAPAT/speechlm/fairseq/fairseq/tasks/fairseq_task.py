...
from fairseq.tasks.aug import Aug_guide
import copy
from fairseq.tasks.wapat import wapat_generator
from fairseq.tasks.compression import *
from fairseq.tasks.decompression import *
...
class FairseqTask(object):
    ...
    def build_model(self, cfg: FairseqDataclass, from_checkpoint=False, spee=None):
        """
        ...
        if spee is not None:
            model = models.build_model(cfg, self, from_checkpoint, spee)
        else:
    ...
    def train_step(
        self, sample, model, criterion, optimizer, update_num, ignore_grad=False, es=None
    ):
        model.train()
        model.set_num_updates(update_num)
        adv_training = 1
        adv_training_embedding = 1
        feature_em_loss = 0
        rir_aug = 0
        isapart_target = 0
        score_match = 0
        sample_o = copy.deepcopy(sample)
        sample_oo =  copy.deepcopy(sample)
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                if adv_training == 1:
                    sample['net_input']["feature_em"] = None
                    advinput, _, res_lprobs_room, feature_em_room = wapat_generator(sample, model, optimizer, AMPOptimizer, attack_criterion=criterion)
                    if adv_training_embedding == 1:
                        sample_oo['net_input']["feature_em"] = advinput.detach().type_as(sample_o['net_input']['source'])
                    else:
                        sample_oo['net_input']['source'] = advinput.detach().type_as(sample_o['net_input']['source'])
                    loss_adv, sample_size, logging_output, _, res_lprobs = criterion(model, sample_oo)
                    if feature_em_loss == 1:
                        rir_loss = -1 * torch.cosine_similarity(res_lprobs.permute(1,0,2), res_lprobs_room.detach().permute(1,0,2)).mean()
                        loss_adv += rir_loss
                    sample_o['net_input']["feature_em"] = feature_em_room
                    loss_clean, sample_size, logging_output, _, _ = criterion(model, sample_o)
                    print(loss_clean)
                else:
                    loss_adv = 0
                    sample_o['net_input']["feature_em"] = None
                    loss_clean, sample_size, logging_output, _, _ = criterion(model, sample_o)
                if adv_training == 1:
    ...
    def sample_aug(self, sample_room):
        import random
        a = random.random()

        if a < 0.2:
            aug_guide = Aug_guide()
            sample_room = aug_guide.aug_RIR(sample_room)
            ...
            sample_room['net_input']['source'] = noise_audio * 0.3 + sample_room['net_input']['source']
        else:
