...
import sys
sys.path.append('/root/Plug-and-Play-Attacks/LSGM/')
...
class NVAE(nn.Module):
    def __init__(self, arch_instance):
        super(NVAE, self).__init__()
        self.arch_instance = arch_instance
        self.dataset = 'celeba_256'
        self.crop_output = self.dataset in {'mnist', 'omniglot'}
        
        self.log_sig_q_scale = torch.Tensor([5.]).to('cuda:0')
        self.num_bits = 8
        self.decoder_dist = 'dml'

        self.num_latent_scales = 1         # number of spatial scales that latent layers will reside
        self.num_groups_per_scale = 10   # number of groups of latent vars. per scale
        self.num_latent_per_group = 20   # number of latent vars. per group
        self.groups_per_scale = groups_per_scale(self.num_latent_scales, self.num_groups_per_scale)

        # encoder parameteres
        self.num_channels_enc = 32
        self.num_preprocess_blocks = 1  # block is defined as series of Normal followed by Down
        self.num_preprocess_cells = 3    # number of cells per block
        ...
        self.num_cell_per_cond_dec = 1  # number of cell for each conditional in decoder
    ...
    def forward(self, x): #8,3,224,224
        ...
        s = s.expand(batch_size, -1, -1, -1) #8, 64, 128, 128
        for cell in self.dec_tower:
           
            if cell.cell_type == 'combiner_dec':
                # form the encoder
                if idx_dec > 0:
                    tmp = (combiner_cells_s[idx_dec - 1], s)
                ...
                z = self.eps_conv[idx_dec](eps) #8, 20, 112, 112
                # 'combiner_dec'
                s = cell(s, z)
                idx_dec += 1
            else:
        ...
        logits = self.image_conditional(s) #4, 100, 224, 224
