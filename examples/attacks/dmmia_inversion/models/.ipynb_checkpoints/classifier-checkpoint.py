    ...
    def _build_model(self, architecture, pretrained):
        ...
        if 'resnet' in architecture:
            ...
            if architecture == 'resnest50':
                ...
                '''
                checkpoint = torch.load('/root/Plug-and-Play-Attacks/results/resnest101/Classifier_0.9999_no_val.pth',map_location='cpu')['model_state_dict']
                for key in list(checkpoint.keys()):
                    if 'model.' in key:
                        checkpoint[key.replace('model.', '')] = checkpoint[key]
                        del checkpoint[key]
                model.load_state_dict(checkpoint)
                Classifier_0.9648_no_val.pth
                '''
                #model = torch.hub.load('zhanghang1989/ResNeSt',
                                       #'resnest101',
                                       #pretrained=True)
...
#             print(name, submodule)
    ...
    def feature_forward(self, x):
        if type(x) is np.ndarray:
            x = torch.tensor(x, dtype=torch.float).to(self.device)
        for name, submodule in self.model.items():
            print(name, submodule)
        exit()
        out = self.model(x)
        return out
