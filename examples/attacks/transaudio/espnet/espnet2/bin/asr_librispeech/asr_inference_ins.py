...
class Speech2Text_attack:
    ...
    def __call__(
        ...
        word_ = name[t]
        p_flag = len(word_)
        target_text_remade = ''.join(target_text_pre[:position])  + word_.upper() + ' ' + ''.join(target_text_pre[position:])
        ...
        #adv = adv.mul(hanming)
        position_left = position_s-500
        position_right = position_s-500 + adv.shape[0]
        haming = 1
        if haming:
            adv_left = batch["speech"][0][:position_left-500].cuda()
            ...
            adv_right = batch["speech"][0][position_left-500:].cuda()
            ...
            speech_input = torch.cat((batch["speech"][0][:position_left-500].cuda(), adv.cuda(),  batch["speech"][0][position_left-500:].cuda()),0)
