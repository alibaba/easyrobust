...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        # fcos centerness
        self.enable_centerness = args.enable_centerness
        if self.enable_centerness:
            self.center_embed = MLP(hidden_dim, hidden_dim, 1, 3)
            if with_box_refine:
                self.center_embed = _get_clones(self.center_embed, num_pred)
            else:
        ...
        # iou-aware
        self.enable_iouaware = args.enable_iouaware
        if self.enable_iouaware:
    ...
    def forward(self, samples: NestedTensor):
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, \
            enc_outputs_center_unact, enc_outputs_proposals_unact, enc_outputs_iou_unact, \
            enc_token_class_unflat = self.transformer(srcs, masks, pos, query_embeds)
        ...
        if self.enable_centerness:
            ...
            for lvl in range(hs.shape[0]):
                ...
                if reference.shape[-1] == 4:
                    ...
                    assert reference.shape[-1] == 2
                outputs_refpts.append(reference.sigmoid())
                outputs_centers.append(self.center_embed[lvl](hs[lvl]))
                if self.info_mode:
                    break
            outputs_center = torch.stack(outputs_centers)
            outputs_refpt = torch.stack(outputs_refpts)
            out['pred_centers'] = outputs_center[-1]
            out['refpts'] = outputs_refpt[-1]
            if self.aux_loss and not self.info_mode:
                for i, aux in enumerate(out['aux_outputs']):
                    aux['pred_centers'] = outputs_center[i]
                    aux['refpts'] = outputs_refpt[i]
            if self.two_stage and not self.info_mode:
                out['enc_outputs']['pred_centers'] = enc_outputs_center_unact
                out['enc_outputs']['refpts'] = enc_outputs_proposals_unact[..., :2].sigmoid()
        ...
        if self.enable_iouaware:
            ...
            if self.two_stage and not self.info_mode:
                out['enc_outputs']['pred_ious'] = enc_outputs_iou_unact

        if self.enable_tokenlabel and not self.info_mode:
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, losses, focal_alpha=0.25, args=None):
        ...
        self.no_boxrefine_no_twostage = not args.with_box_refine and not args.two_stage
    ...
    def loss_centerness(self, outputs, targets, indices, num_boxes):
        ...
        if self.no_boxrefine_no_twostage:
            target_boxes_ltrb = target_boxes_ltrb.detach()

        losses = {}
        if len(target_boxes_ltrb) == 0:
            losses['loss_center'] = src_centers.sum() * 0  # prevent unused parameters
        else:
            target_centers = compute_centerness_targets(target_boxes_ltrb)
            loss_center = F.binary_cross_entropy_with_logits(src_centers, target_centers, reduction='none')
    ...
    def forward(self, outputs, targets):
        ...
        if 'enc_tokens' in outputs:
            if 'loss_attnaware' not in self.weight_dict:
                l_dict = self.loss_tokens(outputs['enc_tokens'], targets, num_boxes)
            else:
                l_dict = self.loss_attnaware(outputs['enc_tokens'], targets, num_boxes)
            losses.update(l_dict)
...
def build(args):
    ...
    criterion = SetCriterion(num_classes, matcher, weight_dict, losses, focal_alpha=args.focal_alpha, args=args)
    criterion.to(device)
